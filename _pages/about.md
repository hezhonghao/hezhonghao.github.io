...
- Will we be experiencing an LLM-induced value lock-in, due to the reinforcing feedback loop established between humans and AI where LLMs are trained on human preference data that is already affected by LLMs' errors and biases and then reabsorb that altered beliefs? If so, how do we prevent it from happenning? (See this [manuscript](https://tinyurl.com/lockinhypothesis) for a technical paper we propose "the lock-in" hypothesis, where we study lock-in with data analysis of WildChat, simulations, and formal modeling.)
